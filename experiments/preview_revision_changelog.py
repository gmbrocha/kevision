"""Generate a real revision-changelog preview from `workspace_demo/`.

Usage:
    python experiments/preview_revision_changelog.py
...
runs the revision changelog exporter, and prints the resulting xlsx path. The original
...
from backend.deliverables.revision_changelog_excel import write_revision_changelog
from backend.workspace import WorkspaceStore

REPO = Path(__file__).resolve().parents[1]
SOURCE = REPO / "workspace_demo"
OUT = REPO / "experiments" / "revision_changelog_preview.xlsx"
LIMIT = 12
...
def main() -> None:
    tmp_dir = Path(mkdtemp(prefix="revision_changelog_preview_"))
    ...
    write_revision_changelog(store, OUT)
