...
`docs/CURRENT_STATE.md` as the project pulse. Command Center is read-only; the
user or project agent updates repository files, current-state docs, and
manifests when explicitly instructed.
...
`Recent Activity` entry, and update `Current Status`, `Known Gaps`,
`Next Actions`, or `Service Notes` only when materially changed. If the change
is metadata-only, describe it as metadata-only. Do not invent project state; use
`Unverified`, `Not reviewed`, or an explicit uncertainty note when needed.

## Design Review Workflow

For any meaningful visual/layout change, run three passes:

1. Implementation pass
2. Adversarial review pass
3. Synthesis/refinement pass

The adversarial pass should challenge whether the design has drifted into SaaS cheese, unnecessary chrome, weak hierarchy, fake polish, or brand-inconsistent choices.

The synthesis pass should preserve useful critique, reject overcorrection, and apply only grounded improvements.

Then, populate `docs/design/REVIEW_LOG.md` after each design pass with findings.
