...
- [x] **Real-world Excel template** — ~~There isn't one~~. **GOT IT 4/21:** `docs/anchors/mod_5_changelog.xlsx`. Three rows + three embedded screenshot crops. Schema captured in `docs/kevin_changelog_format.md`.
...
## New questions raised by `docs/anchors/mod_5_changelog.xlsx`

- [x] **What is "Mod 5"?** — Yes: typically a Mod has a revision set. However, an additional revision set can also be sent for that same Mod. In that case the first set still needs to be priced, but the later revision may no longer have clouds; only the keynote near the detail may indicate the earlier change.
- [x] **Sub-item rollup rule** — Flexible. He grouped AE-110 that way because he wanted to; there is no hard rule as long as the deliverable is clear and minimizes ambiguity.
- [x] **Duplicate-callout collapse** — Collapse duplicate notes inside the same detail; they do not need to be listed twice. Duplicate notes on other drawings/details should still be called out if clouded, because the work may be in a different project location. If a detail is clearly correlated back to the original detail and the notes are the same, it can read `See XX Detail for reference` so it is still surfaced and checked.
- [x] **Correlation ID** — No strong stability requirement. It just needs to tie related rows together. `page.detail` like `110.4` made sense to him, but it does not need to remain stable across re-issues.
- [x] **Detail-View crop dimensions** — No target pixel size or aspect ratio. The priority is legibility. Crops can be larger, and the report can use fewer items per page if needed to keep detail notes readable.
